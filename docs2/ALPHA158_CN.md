...
| `CLOSE0` | `$close/$close = 1` | 收盘价基准（隐含，通常不显式包含） |
...
| 特征类型 | 窗口 | 数量 | 说明 |
|----------|------|------|------|
| **SUMP** | 5,10,20,30,60 | 5个 | 上涨日成交量总和 |
| **SUMN** | 5,10,20,30,60 | 5个 | 下跌日成交量总和 |
| **SUMD** | 5,10,20,30,60 | 5个 | 上涨下跌成交量差 |
| **VMA** | 5,10,20,30,60 | 5个 | 成交量移动平均 |
| **VSTD** | 5,10,20,30,60 | 5个 | 成交量标准差 |
| **WVMA** | 5,10,20,30,60 | 5个 | 成交量加权均价波动 |

#### 公式示例

```python
# SUMP - 上涨日成交量
SUMP20 = Sum(Greater($close - Ref($close, 1), 0) * $volume, 20)
# 统计上涨日的成交量总和

# SUMN - 下跌日成交量
SUMN20 = Sum(Greater(Ref($close, 1) - $close, 0) * $volume, 20)
...
### 价格特征 (5个)
OPEN0, HIGH0, LOW0, VWAP0, (CLOSE0=1)

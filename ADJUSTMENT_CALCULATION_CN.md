...
**后复权（Backward Adjustment）** - 保持最新价格不变，调整历史价格
...
**前复权（Forward Adjustment）** - 保持历史价格不变，调整最新价格
...
| 特性 | 前复权 (Forward) | 后复权 (Backward) |
|-----|-----------------|------------------|
| **保持不变** | 历史价格 | 最新价格 |
| **调整部分** | 最新价格 | 历史价格 |
| **价格范围** | 可能超过实际交易价 | 接近实际交易价 |
| **适用场景** | 查看历史成本 | 技术分析、回测 |
| **Qlib支持** | ❌ 不支持 | ✅ 支持 |
...
def calculate_cumulative_factor(adjustments):
    """
    计算累积复权因子（后复权）
...
factors = calculate_cumulative_factor(adjustments)
# 输出: {
#   '2020-06-15': 0.6333...,
...
**答**:
1. **技术分析标准**: 后复权是技术分析和量化回测的行业标准
2. **最新价格真实**: 后复权保持最新价格不变，便于实盘对照
3. **避免混淆**: 统一使用后复权，避免不同复权方式导致的混淆
4. **灵活性**: 通过 `$factor` 字段，用户可以自行计算前复权或原始价格
